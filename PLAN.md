...
`jex` = **SDKMAN 的版本管理** + **jbang 的一键运行** + **Coursier 的依赖解析** + **JDK 诊断工具(jstat/jcmd/jfr)的友好封装**,收进一个二进制。

对应 uv/bun 的能力拆解:

| uv / bun 能力 | jex 对应 |
|---|---|
| 版本管理 `uv python install` | `jex jdk install/use` |
| 依赖解析(快、可缓存) | `jex add`(底层 Coursier) |
| 一键运行 `uv run` / `bun x` | `jex run Main.java` |
| 包仓库 | Maven Central(直接连) |
| 工具链扩展(bun test/bundle) | `jex java gc/threads/heap/flame` |
| 导出到主流生态 | `jex export maven` → `pom.xml` |
...
| **async-profiler** | 原生火焰图 | `jex java flame` 捆绑它 |
...
> **关键判断**:工具自己用 Rust 写(免 JDK),但 `jex run` 需要**目标 JDK** —— 那由 M1 的 `jex jdk` 负责提供。两层分离后就没有鸡生蛋问题。
...
[project]
name = "demo"
java = "21"                 # 目标 JDK 版本(可锁定,影响 jex run 用哪个 JDK)
...
- 解析结果(含传递依赖)缓存到 `~/.jex/cache`,并落一份锁文件 `jex.lock.toml`(对标 `uv.lock`)。
...
### M0 — 骨架 + CLI 框架
- **目标**:`jex --help` 列出所有子命令占位;`~/.jex` 配置目录就绪。
- **复用**:`clap`、`anyhow`。
- **自研**:项目初始化(cargo)、错误体系、配置目录。
- **验收**:能 `cargo run -- --help` 看到 `jdk / add / run / java / export` 子命令。

### M1 — JDK 版本管理 `jex jdk`
- **目标**:列可装/已装、`install 21` 下载 Temurin、`use 17` 切换(写 `.jex-version`)、`jex which java` 返回当前 JDK 的 `java` 路径。
- **复用**:Adoptium API(或 shell SDKMAN)。
- **自研**:下载/解压/缓存/版本切换逻辑。
- **验收**:装 21 后 `jex which java` 指向它;`jex run` 用对版本。

### M2 — uv 式依赖 + 一键运行 `jex init/add/run`
- **目标**:`init` 生成 `jex.toml`;`add <coord>` 写入并调 Coursier 解析、缓存 jar;`run Main.java` 解析→拼 classpath→`javac`→`java`,编译产物缓存。
- **复用**:Coursier(`cs`)。
- **自研**:manifest 读写、classpath 拼接、编译缓存。
- **验收**:写个用 gson 的 `Main.java`,`jex run` 能跑;二次运行命中缓存、不重新解析。

### M3 — 导出 Maven `jex export maven`
- **目标**:读解析后的依赖树 → 生成 `pom.xml`。
- **自研**:依赖树 → pom 模板(注意**有损**,见 §7)。
- **验收**:生成的 `pom.xml` 能 `mvn compile` 通过。

### M4 — `java` 命令拓展:诊断包装 `jex java ...`
- **目标**:
  - `jex java gc <pid>` —— `jstat -gcutil` 美化 + 列解释
  - `jex java threads <pid>` —— `jstack`/`jcmd Thread.print`
  - `jex java heap <pid>` —— `jmap` / JFR 堆概览
  - `jex java flame <pid>` —— async-profiler 火焰图(HTML)
- **复用**:JDK 自带工具 + async-profiler。
- **自研**:输出解析/美化。
- **验收**:对跑着的 Java 进程出可读报告。

### M5 — 深度 GC 分析(JFR)
- **目标**:`jex java rec <pid>` 起 Flight Recorder,停下后解析 JFR(分配压力 / GC 停顿拆解 / 热点方法)出分析报告。
...
1. **不能真的给 `java` 加子命令。** `java` 是 JDK 自带二进制,无法插入 `gc` 子命令,除非 PATH 劫持。正确做法:`jex java gc ...` 二级命令,顶多提供可选 `alias java=jex-java`。设计里明说,不承诺「扩展 java 原生命令」。
2. **导出 pom 是有损的。** uv 能导出 `requirements.txt` 因两边都是扁平清单;Maven 有 `<scope>`/`<exclusions>`/`<profile>`/多模块,扁平 `jex.toml` 只能生成「种子 pom」——让人能 `mvn` 接着跑,非全等往返。定位成**降低退出成本**,不是双向同步。
...
从 **M0 + M1** 起步:先立起 Rust 骨架和「JDK 版本管理」——这一步不依赖 Coursier、不依赖诊断,最容易跑通且立刻有用(`jex jdk install 21 && jex which java`)。
