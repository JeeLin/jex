...
### v0.25.0 依赖依赖树增强 ✅ 已完成
- **核心功能**：`jex tree --verbose` 显示依赖树详细信息
- **子任务预估**：3 个（树增强核心、CLI 集成、测试补充）
- **依赖**：v0.24.0
- **版本类型**：minor
- **版本号**：v0.25.0

### v0.26.0 热重载 ✅ 已完成
- **核心功能**：`jex watch` 文件变更自动重新编译运行（开发体验）
- **子任务预估**：3 个（文件监控核心、CLI 集成、测试补充）
- **依赖**：v0.25.0
- **版本类型**：minor
- **版本号**：v0.26.0

### v0.27.0 Monorepo 支持 ✅ 已完成
- **核心功能**：`jex workspace` 多模块项目统一管理（依赖共享、批量操作）
- **子任务预估**：3 个（workspace 核心、CLI 集成、测试补充）
- **依赖**：v0.26.0
- **版本类型**：minor
- **版本号**：v0.27.0

### v0.28.0 文档与测试覆盖率 ← 新增（下一步）
- **核心功能**：补充项目文档 + 测试覆盖率提升至 100%
- **子任务预估**：3 个（API 文档补充、测试覆盖率提升、覆盖率验证）
- **依赖**：v0.27.0
- **版本类型**：patch
- **版本号**：v0.28.0
